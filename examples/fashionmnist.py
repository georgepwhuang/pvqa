...
observable_list = list(qml.pauli.pauli_group(3))

kmnist_train_dataset = datasets.FashionMNIST(root=DATA_DIR, train=True, download=True)
kmnist_test_dataset = datasets.FashionMNIST(root=DATA_DIR, train=False, download=True)
pca_train_dataset, pca_test_dataset = convert_mnist_data(kmnist_train_dataset, kmnist_test_dataset, 8)
pvq_dataset = convert_to_post_variational(pca_train_dataset, 3, observable_list, qml.AmplitudeEmbedding,
                                          embedding_kwargs={"normalize": True}, processing_batch_size=512)
test_dataset = convert_to_post_variational(pca_test_dataset, 3, observable_list, qml.AmplitudeEmbedding,
                                           embedding_kwargs={"normalize": True}, processing_batch_size=512)
...
model = FeedForwardClassifier(len(observable_list), len(observable_list), 10, 1e-3)
